...
import { SendPackageResponseDto } from "./dto/sendPackageResponse.dto";
...
export class PackageBillingService {
    ...
    async sendPackage(ebSystemDto:EbSystemDto,packageId:number){
        ...
        await Promise.all(ebPackageBill.bills.map( async (bill) => {
            ...
            const xml = await this.billXmlService.createBillXml(ebSystemDto, ebBillDto, ebSectorDocumentDto, sbSucursalDto);
