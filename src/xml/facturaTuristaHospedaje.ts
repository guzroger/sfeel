import { EbTuristAccommodationDto } from "src/model/dto/ebTuristAccommodation.dto";
import { EbSucursalDto } from "../model/dto/ebSucursal.dto";
import { EbCommercialExportDto } from "src/model/dto/ebComertialExport.dto";
export class FacturaTuristicaHospedaje {
  static facturaElectronicaTuristicaHospedaje(
    data: EbTuristAccommodationDto,
    ...
    <facturaElectronicaServicioTuristicoHospedaje xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                                                  xsi:noNamespaceSchemaLocation="facturaElectronicaServicioTuristicoHospedaje.xsd">
        <cabecera>
        ...
        ${
          ...
          <codigoCliente>${data.clientCode}</codigoCliente>
            <razonSocialOperadorTurismo>${
              data.reasonSocialTourismOperator
            }</razonSocialOperadorTurismo>
            <cantidadHuespedes>${data.GuestQuantity}</cantidadHuespedes>
            <cantidadHabitaciones>${data.quantityRooms}</cantidadHabitaciones>
            <cantidadMayores>${data.quantityMajors}</cantidadMayores>
            <cantidadMenores>${data.minorQuantity}</cantidadMenores>
            <fechaIngresoHospedaje>${
              data.lodgingEntryDate
            }</fechaIngresoHospedaje>
            ...
            ${
              ...
              ${
                data.amountGifCard
                  ? `  <montoGiftCard>${data.amountGifCard}</descuentoAdicional>`
        ...
        ${data.details.map((item) => {
          return `<detalle>
                      ...
                      <codigoTipoHabitacion>${item.roomTypeCode}</codigoTipoHabitacion>
                      ...
                      <detalleHuespedes>${item.guestDetail}
                      </detalleHuespedes>
                 </detalle>`;
        }).join(`
          `)}        
    </facturaElectronicaServicioTuristicoHospedaje>
    `;
    return xml;
  }

  static facturaComputarizadaTuristicaHospedaje(
    data: EbTuristAccommodationDto,
    ebSucursalDto: EbSucursalDto
  ): string {
    let dateEmiite = data.dateEmiite.toISOString();
    ...
    <facturaComputarizadaServicioTuristicoHospedaje xsi:noNamespaceSchemaLocation="facturaComputarizadaServicioTuristicoHospedaje.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
        <cabecera>
        <nitEmisor>${data.nitEmitter}</nitEmisor>
        <razonSocialEmisor>${data.billNameEmitter}</razonSocialEmisor>
        <municipio>${ebSucursalDto.municipality}</municipio>
        ...
        ${data.details.map((item) => {
          return `<detalle> 
                    ...
                    `)}
    </facturaComputarizadaServicioTuristicoHospedaje>
    `;
    return xml;
  }
}
